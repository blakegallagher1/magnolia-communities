        ...
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                if sources_to_refresh:
                    logger.info(
                        f"Re-ingesting {len(sources_to_refresh)} stale sources"
                    )
                    ...
                    for source_name in sources_to_refresh:
                        ...
                        if method:
                            try:
                                await method()
                                logger.info(
                                    f"Successfully re-ingested: {source_name}"
                                )
                            except Exception as e:
                                logger.error(
                                    f"Failed to re-ingest {source_name}: {e}"
                                )
                        else:
                            logger.warning(
                                f"No ingestion method for: {source_name}"
                            )
                else:
